...
public class Question1 extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        pnlGeyserOptionsLayout.setHorizontalGroup(
            ...
            .addGroup(pnlGeyserOptionsLayout.createSequentialGroup()
                .addGroup(pnlGeyserOptionsLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(pnlGeyserOptionsLayout.createSequentialGroup()
                        .addGap(57, 57, 57)
    ...
    private void btnSalesAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSalesAddActionPerformed
        ...
        int numberBedrooms = Integer.parseInt(txfBeds.getText());
        int numBathrooms = Integer.parseInt(txfBaths.getText());
        double marketValue = Double.parseDouble(txfMarket.getText());
        String output = "";
        if (chkPool.isSelected() == true) {
            output = String.format("House for Sale :\n"
                    + "R%.0f#%dBed#%dBath#Pool#", sellingPrice, numberBedrooms, numBathrooms);
        } else {
            output = String.format("House for Sale :\n"
                    + "R%.0f#%dBed#%dBath#", sellingPrice, numberBedrooms, numBathrooms);
        }
        if (marketValue > sellingPrice) {
            output += "Bargain";
        }
    ...
    private void btnCalcRenovationActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCalcRenovationActionPerformed
        ...
        if (rbtPaint.isSelected()) {
            renovation = 'P';
        } else if (rbtTile.isSelected()) {
            renovation = 'T';
        }
        double area = Double.parseDouble(txfArea.getText());
        if (renovation == 'P') {
            double volume = area / 8;
            ...
            /* Loop Method
             ...
             while (roundedVolume > 5) {
             fiveLitreDums++;
             roundedVolume -= 5;
             }
             while (roundedVolume > 1) {
             twoLitreDrums++;
             roundedVolume -= 2;
             }
             while (roundedVolume <= 1 && roundedVolume > 0) {
             oneLitreDrums++;
             roundedVolume -= 1;
             }
             */
            // Division Method
            fiveLitreDums = (int) roundedVolume / 5;
            int remainder = (roundedVolume % 5);
            twoLitreDrums = (int) remainder / 2;
            remainder = (remainder % 2);
            oneLitreDrums = remainder;

            double totalCost = (oneLitreDrums * 55.50) + (twoLitreDrums * 92.30) + (fiveLitreDums * 199.00);

            String output = String.format("Area : %.0f square metres\n"
                    + "Volume of Paint Required : %.2f litres\n\n"
                    + "1-litre dums: %d\n"
                    + "2-litre drums: %d\n"
                    + "5-litre drums: %d\n"
                    + "\nTotal Cost: R%.2f", area, volume, oneLitreDrums, twoLitreDrums, fiveLitreDums, totalCost);
            txaOutputRenovation.setText(output);

        } else {
            // Tiling
            double cost = Double.parseDouble(JOptionPane.showInputDialog(null, "Enter cost per square metre here : "));
            area += 5.00;
            double tilingCost = cost * area;
            String output = String.format("Area: %d sqaure meters\n"
    ...
    private void btnCalcAmountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCalcAmountActionPerformed
        ...
        int newReading = Integer.parseInt(txfCurrent.getText());

        if (newReading < previousReading) {
            JOptionPane.showMessageDialog(null, "Curent reading is less than previous reading.");
            txfCurrent.setText(null);
        } else if (newReading >= previousReading) {
            int difference = newReading - previousReading;
            double tariff = 0.00;
            double totalCost = 0.00;
            if (difference <= 600) {
                tariff = 1.00;
                totalCost = tariff * difference;
            } else if (difference > 600) {
                tariff = 1.50;
                totalCost += 600;
                difference -= 600;
                totalCost += (difference * tariff);
            }
            lblElectricity.setText(String.format("R %.2f", totalCost));
        }
    ...
    private void btnFindGeysersActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFindGeysersActionPerformed
        txaOutputGeysers.setText(null); 
        int capacity = Integer.parseInt(txfGeyserSize.getText());
        for (String geyser : arrGeysers) {
            String[] details = geyser.split("-");
            if (details[0].equals(capacity + "")) {
