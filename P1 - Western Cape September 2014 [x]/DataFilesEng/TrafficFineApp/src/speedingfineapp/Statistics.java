...
public class Statistics extends javax.swing.JFrame {
    ...
    private void btnDisplayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDisplayActionPerformed
        ...
        for (int i = 0; i < headings.length; i++) {
            txaOutput.append(String.format("%-15s", headings[i]));

        }
        txaOutput.append("\n--------------------------------------------------------------------\n");
        for (int i = 0; i < towns.length; i++) {
            txaOutput.append(String.format("%-15s%-15s%-15s%-15s%-15s\n", towns[i],
                    stats[i][0], stats[i][1], stats[i][2], totals[i]));
        }
    ...
    private void btnHighestActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnHighestActionPerformed
        ...
        for (int i = 0; i < stats.length; i++) {
            for (int j = 0; j < 3; j++) {
                int currentStat = stats[i][j];
                if (currentStat > highest) {
                    highest = currentStat;
                    row = i;
                    column = j;
                }
        ...
        String town = towns[row];
        String month = headings[column + 1];
        txaOutput.append(String.format("\nThe highest amount is R%d recorded in %s for %s", highest, town, month));
