...
//        }
        ...
        for (int i = 0; i < max - 1; i++) {
            int count = 1;
            double total = arrFines[i];
            for (int j = i + 1; j < max; j++) {
                if (arrRegNums[i].equals(arrRegNums[j]) && !arrRegNums[i].equals("X")) { // && multiple[j] == false
                    count++;
                    total += arrFines[j];
                    arrRegNums[j] = "X";
//                    multiple[i] = true;
//                    multiple[j] = true;
            ...
            if (count > 1) {
                String out = String.format("%s%s%d%s", arrRegNums[i], " had ", count, " fines");
                txaOutput.append(String.format("%-31s%sR%.2f%n", out, "Total owing: ", total));
                arrRegNums[i] = "X";
                //(arrRegNums[i] + " had " + count + " fines" + "\tTotal owing: " + String.format("R%.2f", total) + "\n");
            }
        }
...
// 3.2
        ...
        } while (arrFines[ranNum] != 100.0 || arrRegNums[ranNum].equals("X"));  // || multiple[ranNum] == true
