...
public class Question1 extends javax.swing.JFrame {
    ...
    private void btnDeliveryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeliveryActionPerformed
        // Question 1.1
        String departure = cmbDepart.getSelectedItem().toString();
        String destination = cmbDestination.getSelectedItem().toString();
        try {
            distance = Integer.parseInt(txfDistance.getText());
            if (departure.equals(destination)) {
                JOptionPane.showMessageDialog(null, "Please review your destination and departure.");
            } else {
                lblDelivery.setText(String.format("%s to %s : %d km", departure, destination, distance));
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "Please review your distance entered.");
        }
    ...
    private void btnDeliveryCostActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeliveryCostActionPerformed
        // Question 1.2
        if (lstKgs.isSelectionEmpty() == false) {
            int categoryIndex = lstKgs.getSelectedIndex();
            double tariff = 0.00;
            ...
            txfCost.setText(String.format(" R %.2f", cost));
        } else {
    ...
    private void btnBoxNumberActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBoxNumberActionPerformed
        ...
        if (speedPost == true) {
            ...
            while (generatedNumber == 4) {
                Random random = new Random();
                generatedNumber = random.nextInt((5 - 1) + 1) + 1;
            }
            deliveryBox = generatedNumber;
        }
        txfBoxNumber.setText(deliveryBox + "");
    ...
    private void btnCheckDigitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCheckDigitActionPerformed
        ...
        // Logical position of the position of any element in an index.
        for (int i = 0; i < barCode.length() - 1; i++) { // Watch our for i < x errors in loop control
            int digit = Integer.parseInt(Character.toString(barCode.charAt(i)));
            if (((i+1) % 2) == 0) {
                // Even
        ...
        int sum = (sumOddPositions *3) + sumEvenPositions;
        int checkDigit = 10 -(sum %10);
        String output = "";
        if (checkDigit == Integer.parseInt(Character.toString(barCode.charAt(barCode.length()-1)))) {
            output = String.format("The bar code is valid. Check Digit: %d", checkDigit);
        } else {
            output = String.format("The bar code is NOT valid. Correct check digit: %d", checkDigit);
        }
        txfDisplayBarCode.setText(output); 
              
    }//GEN-LAST:event_btnCheckDigitActionPerformed

    private void btnViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnViewActionPerformed
        // Question 1.5
        String city = cmbCityName.getSelectedItem().toString();
        String output = "";
        for (String delivery : arrDecDeliveries) {
            if (delivery.contains(city)) {
                output += String.format("%s\r\n", delivery);
            }
        }
        File file = new File(String.format("December2015%s.txt", city));
        try {
            FileWriter writer = new FileWriter(file); // Will create the file, then write to it
            writer.write(output);
            writer.flush();
            writer.close();
